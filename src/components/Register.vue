    ...
    methods: {
      async handleSubmit() {
        ...
        if (this.user.email && this.user.password) {
            await createUser(this.user.email, this.user.password);
            this.$router.push({
                path: '/publicaciones'
            });
        }
...
<template>
  ...
  <div class="flex items-center justify-center min-h-screen">
    ...
    >
      ...
      <!-- <button class="flex justify-center gap-2 w-full py-4 mt-4 text-purple-700 border-purple-700 border-2 rounded-md">
          <BIconGoogle></BIconGoogle>
      </button> -->
      <div class="text-center mt-5">
        <p>Ya tenes una cuenta? <span @click="goToLogin()" class="text-purple-700 font-bold">Iniciar sesion</span></p>
        </div>
    </form>
